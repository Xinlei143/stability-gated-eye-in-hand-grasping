...
class ControlPhysicsQualification(Node):
    def __init__(self):
        ...
        if self.mode not in ("arm", "gripper"):
            raise RuntimeError("mode must be arm or gripper")
    ...
    def _joint_callback(self, message):
        ...
        self.joints = {
            **{name: float(positions[name]) for name in ARM_JOINTS},
            ...
            "effort7": float(efforts.get("joint7", math.nan)),
            "effort8": float(efforts.get("joint8", math.nan)),
        }
    ...
    def _write_samples(self, cycle, stage, samples):
        ...
        exists = path.exists()
        fields = [
            "cycle", "stage", *ARM_JOINTS,
            "joint7", "joint8", "effort7", "effort8",
        ]
    ...
    def run(self):
        results = self._run_arm() if self.mode == "arm" else self._run_gripper()
